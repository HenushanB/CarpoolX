...
import { findTaxi, user_information } from './viewTripScript'
 ...
 * @returns 
 */
const ViewTripScreen = ({route, navigation}) => {
    const {rideToFind}  = route.params;
    const [taxCode, setTaxCode] = useState(rideToFind)
    ...
    useEffect(() => {
        const fetchData = async () => {
            console.log('Finding taxi with code: ' + rideToFind + '...')
            const taxiInfo = (await findTaxi(rideToFind))["data"]
    ...
    return (
        <View style={styles.root}>
            <View style={styles.container_info}>
                <Text style={styles.title}>Taxi ID #{rideToFind}</Text>
